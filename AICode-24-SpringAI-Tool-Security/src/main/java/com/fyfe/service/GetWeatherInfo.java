...
public class GetWeatherInfo {
    ...
    @Tool(name = "get_user_info", description = "查询用户的详细信息")
    @PreAuthorize("hasRole('ADMIN')") // 当前工具方法必需具体Admin角色才能访问
    public String getUserInfo(@ToolParam(description = "需要查询的用户名") String username) {
        return  " 获取到了"+ username +"的用户信息：姓名为：年龄20"; // 业务方法，模拟调用第三方接口获取天气
    }
